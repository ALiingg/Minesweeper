...
public class Block {
    ...
    public BlockStatus getBlockStatus(boolean gameFinished) {
        ...
        // Setting Global var to local
        int numMineAround = this.numMineAround;
        // if game is finished

        if (gameFinished) {
            //if the block is flagged by user
            if (this.flagged) {
                return BlockStatus.MINE_CORRECT;
                //if it is a mine
            } else if (this.isMine) {
                return BlockStatus.MINE_WRONG;
                // if it is not hidden then return the num of mine around or block status
            } else if (this.isRevealed()) {
                return getMineAroundStatus();
            }
            // if game is continuing
        } else if (!gameFinished) {
            // return flagged if it is flagged
            if (this.isFlagged()) {
                return BlockStatus.FLAGGED;
                //return the block status by calling getMineAroundStatus()
            } else if (this.isRevealed()) {
//				System.out.println(numMineAround);
                return getMineAroundStatus();
            }
        }
