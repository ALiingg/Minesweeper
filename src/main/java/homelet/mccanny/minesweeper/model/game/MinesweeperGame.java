...
public class MinesweeperGame {
    ...
    private void fisherYatesShuffle() {
        ...
        for (int i = 0; i < n - 1; i++) {
            j = i + (int) (random() * (n - i));   // change this line to generate j, such that i <= j < n
    ...
    public int countMinesAround(int rowIndex, int colIndex) {
        ...
        ArrayList<Block> blockAround = new ArrayList<>(getBlocksAround(rowIndex, colIndex));
        for (int i = 0; i < blockAround.size(); i++) {
            if (blockAround.get(i).isMine()) {
                numMinesAround++;
            }
        }
    ...
    public void revealBlock(int rowIndex, int colIndex) {
        ...
        if ((isInvalidPosition(rowIndex, colIndex) || checkReveal(rowIndex, colIndex)) || board[rowIndex][colIndex].isMine() || board[rowIndex][colIndex].isFlagged()) {
            return;
        }
    ...
    /**
     * Returns whether there are mines around the given block or not
     *
     * @param rowIndex row index
     * @param colIndex col index
     * @return boolean
     *
     */
    private boolean isEmptyBlock(int rowIndex, int colIndex) {
        return board[rowIndex][colIndex].getNumMineAround() == 0;
    }

    /**
     * Returns whether the block given is revealed or not
     *
     * @param rowIndex row index
     * @param colIndex col index
     * @return boolean, true if it is Revealed
     */
    public boolean checkReveal(int rowIndex, int colIndex) {
        return board[rowIndex][colIndex].isRevealed();

    }

    /**
     * Sets the given block reveal
     *
     * @param rowIndex row index
     * @param colIndex col index
     */
    public void reveal(int rowIndex, int colIndex) {
        board[rowIndex][colIndex].setRevealed(true);
    }
//    /**
//     * Returns whether is it a valid position or not
//     *
//     * @param rowIndex row index
//     * @param colIndex col index
//     * @return boolean false if the block is invalid
//     *
//     * Note: Two cases, already revealed or out of bound
//     */
//    public boolean isValidPosition(int rowIndex, int colIndex) {
//        return !board[rowIndex][colIndex].isRevealed() && (rowIndex >= level.getBoardHeight() || colIndex >= level.getBoardWidth());
//    }
